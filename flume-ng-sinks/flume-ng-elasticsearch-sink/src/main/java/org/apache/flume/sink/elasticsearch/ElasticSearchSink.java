...
import static org.apache.flume.sink.elasticsearch.ElasticSearchSinkConstants.CONTENT_TYPE;
...
public class ElasticSearchSink extends AbstractSink implements Configurable {
	...
	public void configure(Context context) {
		...
		if (StringUtils.isNotBlank(context.getString(CONTENT_TYPE))) {
			contentType = context.getString(CONTENT_TYPE);
		}
	...
	public void start() {
		...
		try {
			if (isLocal) {
				...
				client = clientFactory.getClient(contentType, clientType, serverAddresses, clusterName, eventSerializer,
						indexRequestFactory);
	 ...
	 * where the number of days is parsed integer from TTL string provided by user.
	 * <p> Elasticsearch supports ttl values being provided in the format: 1d / 1w /
	 * 1ms / 1s / 1h / 1m specify a time unit like d (days), m (minutes), h (hours),
